...
import { PedidoContextPanel } from './pedido-context-panel'
import { PedidoItemEditor } from './pedido-item-editor'
...
export function PedidoFormUnified({ contexto, clientes, onSubmit, pedidoInicial }: PedidoFormUnifiedProps) {
  ...
  // Extraído del onClick inline del botón "Quitar cliente" (Fase 3b,
  // PedidoContextPanel) — mismo reset exacto, sin cambio de comportamiento.
  ...
  return (
    <form onSubmit={handleSubmit} className="lg:grid lg:grid-cols-[1fr_360px] lg:gap-6 h-full">
      ...
      <div className="space-y-4 overflow-y-auto lg:max-h-[calc(90vh-2rem)] pr-1">
        ...
        {/* Cliente (extraído a PedidoContextPanel, ALS §5 — Fase 3b) */}
        <PedidoContextPanel
          canal={canal}
          pedidoInicialId={pedidoInicial?.id}
          clienteSeleccionado={clienteSeleccionado}
          onQuitarCliente={handleQuitarCliente}
          fiadosStatus={fiadosStatus}
          sugerenciaConsumo={sugerenciaConsumo}
          sugerenciaLoading={sugerenciaLoading}
          sugerenciaAplicada={sugerenciaAplicada}
          aplicarSugerenciaDisabled={productosConfig.length === 0}
          onAplicarSugerencia={aplicarSugerenciaConsumo}
          onVerPatronConsumo={verPatronConsumo}
          ...
          onEditDireccionChange={setEditDireccion}
          editBarrio={editBarrio}
          onEditBarrioChange={setEditBarrio}
          soloParaEstePedido={soloParaEstePedido}
          onSoloParaEstePedidoChange={setSoloParaEstePedido}
        ...
        {/* Productos (extraído a PedidoItemEditor, ALS §5 — Fase 3c) */}
        <PedidoItemEditor
          items={productosActuales.map(prodId => {
            const info = PRODUCTO_INFO[prodId]
            return {
              prodId,
              cantidad: cantidades[prodId] || 0,
              precio: getPrecio(info.codigo),
              precioBase: getPrecioBase(info.codigo),
              precioManual: preciosManuales[info.codigo],
              precioOrigen: preciosOrigen[info.codigo],
              tiers: tablaPrecios[info.codigo] || [],
              precioBajoConfirmado: Boolean(precioBajoConfirmado[info.codigo]),
            }
          })}
          ...
          onPrecioManualChange={setPrecioManual}
          onConfirmarPrecioBajo={(codigo) => setPrecioBajoConfirmado(prev => ({ ...prev, [codigo]: true }))}
        />

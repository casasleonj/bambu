...
// el hook N2 aplica la clasificación centralizada de `conflicto-409` — A
// (conflicto de estado → recovery) vs B (regla de negocio → mensaje
// contextual). No todo 409 es "conflicto". Nunca auto-retry, nunca éxito.
...
describe('useGestionPendiente — clasificación de 409 (F9-iii)', () => {
  // ── B: regla de negocio, sin recovery ──
  it('409 CANTIDAD_EXCEDE_PENDIENTE → regla de negocio, NO conflicto', async () => {
    frMock.mockResolvedValue({ status: 'error', statusCode: 409, error: 'CANTIDAD_EXCEDE_PENDIENTE: quedan 2' })
    const { r, onMutado } = await confirmar()
    expect(r.ok).toBe(false)
    expect(r.reglaNegocio).toBe(true)
    expect(r.conflicto).toBe(false)
    expect(onMutado).not.toHaveBeenCalled()   // nunca éxito
  })

  it('409 ACTIVIDAD_SIN_MODO → regla de negocio, NO conflicto', async () => {
    frMock.mockResolvedValue({ status: 'error', statusCode: 409, error: 'ACTIVIDAD_SIN_MODO: la actividad no tiene modo' })
    const { r } = await confirmar()
    expect(r.reglaNegocio).toBe(true)
    expect(r.conflicto).toBe(false)
  })

  // ── A: conflicto de estado, con recovery ──
  it('409 OBLIGACION_YA_ACTIVA → conflicto de estado', async () => {
  ...
  it('409 ACTIVIDAD_NO_MODIFICABLE → conflicto de estado', async () => {
    frMock.mockResolvedValue({ status: 'error', statusCode: 409, error: 'ACTIVIDAD_NO_MODIFICABLE: ya está cancelada' })
    const { r } = await confirmar()
    expect(r.conflicto).toBe(true)
  })

  it('409 con código desconocido → conflicto de estado (default seguro)', async () => {
    frMock.mockResolvedValue({ status: 'error', statusCode: 409, error: 'ALGO_INESPERADO: ...' })
    const { r } = await confirmar()
    expect(r.conflicto).toBe(true)
    expect(r.reglaNegocio).toBe(false)
  })
  ...
  // ── no-409 ──
  it('500 → error técnico: ni conflicto ni regla de negocio', async () => {
  ...
  it('403 → ni conflicto ni regla de negocio', async () => {
    frMock.mockResolvedValue({ status: 'error', statusCode: 403, error: 'No autorizado' })
    const { r } = await confirmar()
    expect(r.conflicto).toBe(false)
    expect(r.reglaNegocio).toBe(false)
  })
  ...
  it('un 409 A NO dispara un segundo commit (sin auto-retry en el hook)', async () => {
    frMock.mockResolvedValue({ status: 'error', statusCode: 409, error: 'ACTIVIDAD_NO_MODIFICABLE: ...' })
    await confirmar()

...
export default function NuevoRecurrentePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const totalPacas =
      formData.pacaAgua +
      formData.pacaHielo +
      formData.botellonFab +
      formData.botellonDom +
      formData.bolsaAgua +
      formData.bolsaHielo

    if (totalPacas === 0) {
      toast.error('Debes especificar al menos un producto')
      return
    }
    ...
    try {
      const res = await fetch('/api/recurrentes', {
        ...
        body: JSON.stringify({
          ...
          productos: {
            pacaAgua: formData.pacaAgua,
            pacaHielo: formData.pacaHielo,
            botellonFab: formData.botellonFab,
            botellonDom: formData.botellonDom,
            bolsaAgua: formData.bolsaAgua,
            bolsaHielo: formData.bolsaHielo,
          },
          obs: formData.obs,
        }),
      })
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {[
                ...
                { key: 'botellonFab', label: 'Bot. Fábrica' },

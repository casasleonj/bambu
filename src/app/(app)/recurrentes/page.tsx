...
export default function RecurrentesPage() {
  ...
  async function fetchData() {
    ...
    try {
      ...
      if (previewData.success) {
        ...
        setPreview(p)
        // Default decisions to NORMAL
        const defaults: Record<string, string> = {}
        for (const item of p) {
          defaults[item.recurrenteId] = 'NORMAL'
        }
  ...
  async function handleGenerar() {
    ...
    if (decisionesArray.length === 0) {
      toast.error('No hay decisiones para generar')
      return
    }
    ...
    try {
      ...
      if (data.success) {
        toast.success(`${data.generados} generados, ${data.saltados} saltados`)
        fetchData()
      } else {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Pedidos Recurrentes</h1>
          <p className="text-gray-600">{recurrentes.length} patrones activos</p>
        </div>
        <button
          onClick={() => router.push('/recurrentes/nuevo')}
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-lg font-semibold text-yellow-800">
              Generación para hoy ({formatDate(new Date().toISOString())})
            </h2>
            <button
              ...
              <div key={item.recurrenteId} className="bg-white rounded-lg border p-3">
                  ...
                  <div>
                    ...
                    {item.pedidosPendientes.length > 0 && (
                      <span className="text-xs text-red-600 ml-2">
                        ({item.pedidosPendientes.length} pendientes)
                      </span>
                    )}
                  </div>
                </div>
                  ...
                  <div className="text-sm text-gray-600 mb-2 bg-red-50 p-2 rounded">
                    ...
                    {item.pedidosPendientes.map((p) => (
                      <span key={p.id} className="ml-2">
                        #{p.numero} ({p.cPacaAguaPed + p.cPacaHieloPed + p.cBotellonFabPed + p.cBotellonDomPed + p.cBolsaAguaPed + p.cBolsaHieloPed} pacas)
                      </span>
                    ))}
                  </div>
                )}
                ...
                <div className="flex gap-2 flex-wrap">
                  {item.sugerencias.map((sug) => (
                    ...
                    >
                      {sug.label}
                      <span className="block text-xs font-normal opacity-90">{sug.descripcion}</span>
                    </button>
                  ))}
                </div>

                {item.saltarFechas.length > 0 && (
                  <p className="text-xs text-gray-500 mt-2">
                    Saltos programados: {item.saltarFechas.join(', ')}
                  </p>
                )}
              </div>
            ))}
        ...
        {recurrentes.map((r) => (
          <div key={r.id} className="bg-white rounded-lg shadow-sm border p-4">
            ...
            <div className="text-xs text-gray-500 mb-3">
              {r.ultimaGeneracion ? (
                <p>Última generación: {formatDate(r.ultimaGeneracion)}</p>
              ) : (
                <p>Sin generar aún</p>
              )}
              <p>{r._count.pedidoHijo} pedidos generados</p>
              {r.saltarFechas.length > 0 && (
                <p className="text-yellow-600">Saltos: {r.saltarFechas.length}</p>
              )}
            </div>

            <div className="flex gap-2">
              <button
                onClick={() => router.push(`/recurrentes/${r.id}`)}
      ...
      {recurrentes.length === 0 && (
        <EmptyState
          ...
          description="Crea templates de pedidos que se generen automáticamente"

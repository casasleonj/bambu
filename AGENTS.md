...
19. **`/clientes`: negocios stale en panel + filtros lentos + resultados confusos** (resuelto):
  ...
  20. **E2E auth flakiness con workers>1** (resuelto para el login race; DB-reset race mitigado pero no eliminado): Playwright con `workers:1` era estable, pero con `workers>1` tests del mismo rol (ej. `ADMIN`) se evitaban mutuamente por el LRU de `SesionActiva` (`ADMIN=2`, `CONTADOR=1`). El `CredentialsSignin` / 403 intermitente en dev mode venía de logins concurrentes del mismo rol más que de Auth.js en sí. `src/lib/session-limits.ts` ya leía `SESSION_LIMITS_JSON` del entorno para sobreescribir límites (`playwright.config.ts` lo inyecta en `webServer.env`), pero eso solo neutraliza la eviction — el login-race en sí (cientos de POSTs reales concurrentes a `/api/auth/callback/credentials`) seguía sin resolverse.
   ...
   25. **⚠️ ACTIVO — job `e2e` desactivado temporalmente en CI** (`if: false` en `.github/workflows/ci.yml`, agregado 2026-08-20): pedido explícito del usuario mientras dure la limpieza de deuda de lint (Fase 0 del plan de convergencia, ~491 issues restantes tras PR #78). Los 8 shards vienen fallando de forma consistente en 3 PRs seguidas (#76, #77, #78) con la misma causa raíz ya diagnosticada (fallos amplios y sin relación entre sí — `roles-permisos`, `race-conditions`, `security-fixes/pagar-fiado-rbac`, `offline-sync`, `user-flow` — capacidad/timeout de CI, no relacionado a ningún diff de lint); esperar el run completo en cada incremento no aportaba señal nueva. **`Type check + Tests` y `Lint (non-blocking)` siguen corriendo normalmente** — solo el job `e2e` está apagado. **REVERTIR el `if: false` en cuanto la deuda de lint esté resuelta** — no es un cambio permanente. También queda pendiente, una vez reactivado, investigar la causa raíz real del patrón de capacidad (ver #20 para el contexto de aislamiento E2E ya documentado — puede o no ser la misma causa).

...
19. **`/clientes`: negocios stale en panel + filtros lentos + resultados confusos** (resuelto):
  ...
  20. **E2E auth flakiness con workers>1** (resuelto para el login race; DB-reset race mitigado pero no eliminado): Playwright con `workers:1` era estable, pero con `workers>1` tests del mismo rol (ej. `ADMIN`) se evitaban mutuamente por el LRU de `SesionActiva` (`ADMIN=2`, `CONTADOR=1`). El `CredentialsSignin` / 403 intermitente en dev mode venía de logins concurrentes del mismo rol más que de Auth.js en sí. `src/lib/session-limits.ts` ya leía `SESSION_LIMITS_JSON` del entorno para sobreescribir límites (`playwright.config.ts` lo inyecta en `webServer.env`), pero eso solo neutraliza la eviction — el login-race en sí (cientos de POSTs reales concurrentes a `/api/auth/callback/credentials`) seguía sin resolverse.
     ...
     - **workers landed**: `playwright.config.ts` ahora resuelve `workers` como `PW_WORKERS` (override manual) > `CI ? 2 : 1`. CI corre con `workers:2` (dentro de cada uno de los 8 shards existentes, sin tocar el sharding); local sigue en `1` por default. La elección de `2` (no 4/8) es deliberadamente conservadora dado el riesgo residual arriba — crece con la cantidad de workers, así que se prefirió el valor más chico que aún duplica el throughput de CI.
     - **Limitación de validación conocida**: el sandbox de este cambio no pudo descargar el binario de Chromium de Playwright (`cdn.playwright.dev` bloqueado por política de egress de la organización, confirmado vía `/__agentproxy/status` → `connect_rejected`), y no hay Chromium de sistema utilizable (el paquete `chromium-browser` de apt es un stub de snap sin `snapd` funcional en el contenedor). No fue posible correr la suite E2E real con browser en este sandbox. Se validó en su lugar, contra un Postgres+Redis+Next dev server locales reales: (a) el mecanismo de cache de auth completo, incluyendo el self-healing tras un reset real; (b) que los 4 roles validan correctamente contra `/api/config`; (c) que 3 procesos OS concurrentes reseteando la DB vía `reset-locked.ts` serializan sin error; (d) `npx tsc --noEmit` y `npm run test` (2423 tests) pasan. La suite E2E real con browsers **no se corrió**; el primer run real es el propio CI de este PR — revisar sus resultados antes de mergear.

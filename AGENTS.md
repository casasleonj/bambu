...
19. **`/clientes`: negocios stale en panel + filtros lentos + resultados confusos** (resuelto):
  ...
  20. **E2E auth flakiness con workers>1** (resuelto parcialmente): Playwright con `workers:1` era estable, pero con `workers>1` tests del mismo rol (ej. `ADMIN`) se evitaban mutuamente por el LRU de `SesionActiva` (`ADMIN=2`, `CONTADOR=1`). El `CredentialsSignin` / 403 intermitente en dev mode venía de logins concurrentes del mismo rol más que de Auth.js en sí.
     - **Fix**: `src/lib/session-limits.ts` ahora lee `SESSION_LIMITS_JSON` del entorno para sobreescribir límites sin tocar código. `playwright.config.ts` inyecta `SESSION_LIMITS_JSON={ADMIN:50,ASISTENTE:50,CONTADOR:50,REPARTIDOR:50,SELLADOR:50}` en `webServer.env`.
     - **Riesgo residual**: `workers>1` expone una carrera de DB reset preexistente. Varios spec files llaman `resetDatabase()`/`resetTestDatabase()` al cargar el módulo; con workers>1 un worker puede limpiar la DB mientras otro ejecuta un test. Para estabilidad total con workers>1 se requiere aislamiento de DB por worker (p. ej. `bambu_worker_1`, `bambu_worker_2`). `workers:1` sigue siendo el default conservador para CI; el override permite paralelismo local cuando se necesita y se cuenta con aislamiento.
     - **Validación**: `e2e/clientes.spec.ts` corre con `--workers=2`: 105/124 pasan; los 3 fallos son por carrera de DB reset / timeout de login bajo carga, no por eviction de sesión. El test aislado "crear cliente via UI modal" pasa consistentemente con `--workers=2`.

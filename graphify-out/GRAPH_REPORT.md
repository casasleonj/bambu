...
6. `main()` - 22 edges
7. `Pedidos API Route` - 19 edges
8. `Auth Check Library` - 17 edges
9. `PUT()` - 16 edges
10. `DELETE()` - 16 edges

## Surprising Connections (you probably didn't know these)
- `POST /api/facturas - Create factura with advisory lock` --implements--> `PostgreSQL advisory locks for sequence generation`  [INFERRED]
...
- `Trabajadores [id] API Route` → `Embarque Capacity Calculator`  [AMBIGUOUS]
  src/app/api/trabajadores/[id]/route.ts · relation: conceptually_related_to
- `Rate Limiter Service` → `Service Worker Register (PWA)`  [AMBIGUOUS]
  src/components/sw-register.tsx · relation: conceptually_related_to
- `Service Worker Register (PWA)` → `Date Range Filter Component`  [AMBIGUOUS]
...
- **What is the exact relationship between `Trabajadores [id] API Route` and `Embarque Capacity Calculator`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **What is the exact relationship between `Rate Limiter Service` and `Service Worker Register (PWA)`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._

...
| 13 | PROMOCIÓN: sale físicamente, consume disponibilidad, no es venta ordinaria, NO debe aparecer como faltante | **CERRADA** | **BUG/DEFECTO DE INTEGRIDAD — CORREGIDO en PR #267.** `conciliarProductos()` resta las promociones (leídas de `EmbarqueMovimiento`) antes de calcular discrepancia | **P0 — implementado** |
...
**Estado de implementación de este documento**: el P0 (`PROMOCIÓN`) está implementado en **PR #267** (`fix/f5-promocion-conciliacion`) — `conciliarProductos()` ahora resta las unidades promocionales (leídas de `EmbarqueMovimiento`, única fuente del hecho físico) antes de calcular la discrepancia, sin tocar `Pedido`/`PedidoItem` ni el resto del flujo de cierre. 3 tests de integración contra Postgres real, verificados manualmente contra el código sin el fix para confirmar que detectan la regresión real. El resto (P1/P2) sigue sin código, a la espera de que el equipo confirme el orden dentro de P1.

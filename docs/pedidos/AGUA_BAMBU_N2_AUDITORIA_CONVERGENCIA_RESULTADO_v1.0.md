...
| 9 | Materialización real de la planificación es **Planificación → Embarque** (vía `Pedido.embarqueId`), **no** Planificación → Actividad. Contrato completo: `PlanDia CONFIRMED → MaterializarPlanUseCase → CrearEmbarqueUseCase → Embarque ABIERTO → asigna pedidoIds`, con `offlineId` determinista (`hash(planId, version, grupoId)`) y saga de fallo parcial (`INTEGRATION_PARTIAL`, reintentable). | **ADR-PLANIFICADOR-003** (congelado, contrato P0), verificado que `MaterializarPlanUseCase`/`CrearEmbarqueUseCase` existen |
...
| Materialización (Planificación→Embarque) | **DECISIÓN** (cerrada, implementada) | Contrato P0 completo | ADR-PLANIFICADOR-003 | `MaterializarPlanUseCase`/`CrearEmbarqueUseCase` existen | Ninguna | No |
...
**Ya decidido (no reabrir):** el modelo de 4 ledgers, la semántica de cantidades de `ObligacionPendiente`/`Actividad`, que `PlanActividad` y `Actividad` son entidades separadas por diseño, que la materialización real de la planificación es hacia `Embarque` (no hacia `Actividad`), que `saldoFavor` no es reserva de producto, que el dinero nunca se duplica en `Actividad`, que "Gestionar pendiente"/PUNTO→DOMICILIO deben ser explícitos y auditables, y que el diferencial no es una venta nueva.

...
- **Actualización 2026-09-06 (5ª ronda — cierre del punto fiscal)**: el dueño del negocio confirmó directamente la pregunta 0 de §4.3 — Agua Bambú no emite hoy facturación electrónica real ante la DIAN. Esto cierra el diferencial fiscal (§4.7) siguiendo la rama que el propio mandato del equipo ya había previsto: no bloquea el desarrollo actual, la doctrina queda documentada, y el único trigger de reapertura es la futura integración de un proveedor de FE real. **Con esto, no queda ningún punto de este documento pendiente de decisión o de hecho verificable** — lo único vigente hacia adelante es la ejecución del Plan Técnico de rediseño de Fase 3 (§3.7).
...
**Acción**: el siguiente paso concreto NO es escribir código de UI directamente. Conforme al protocolo de este proyecto (`AGENTS.md`: investigación antes de código, aprobación explícita antes de implementar en trabajo de este tamaño) y al precedente real ya existente en este mismo repo (`docs/embarques/00-plan-frontend-completo.md`, el rediseño de Embarques ejecutado en fases con gates), el siguiente entregable es un **Plan Técnico de rediseño de Pedidos** — arquitectura de información, flujos, fases de ejecución (probablemente detrás de `NEXT_PUBLIC_PEDIDOS_V2`, igual que Embarques V2), y gates de verificación — antes de tocar un solo componente.

**Entregado**: [`docs/pedidos/00-plan-frontend-rediseno-integral.md`](00-plan-frontend-rediseno-integral.md) (2026-09-06) — Ronda 1 (hallazgos: mapa de rutas/componentes, contrato de API completo, reglas de negocio que no se reimplementan, deuda transversal, estado de tests) + Ronda 2 (principios, 7 decisiones D1–D7 pendientes de aprobación, 9 fases de PR-1 a PR-9b, verificación y gate). **Estado: pendiente de aprobación del equipo/PO antes de ejecutar PR-1.**
...
### 4.7 Resolución — 🟢 CERRADO por ahora (confirmado por el negocio, 2026-09-06)

**Pregunta 0 respondida directamente por el dueño del negocio**: Agua Bambú **NO emite hoy facturación electrónica real ante la DIAN** — la `Factura` de este sistema es un registro interno de cartera, no un documento fiscal transmitido. Confirma exactamente lo que ya se había verificado desde el código (§4.6: sin CUFE/CUDE/XML UBL, sin NIT en `Cliente`).

Esto activa la rama ya prevista por el propio mandato del equipo (§7 de su mensaje) para el caso "NO existe integración real":

- **El tratamiento fiscal del diferencial NO bloquea el desarrollo actual de Pedidos.** No hay ninguna dependencia entre este punto y cualquier trabajo en curso o planeado (incluyendo el rediseño de Fase 3, §3.7).
- **Se mantiene documentada la conclusión normativa** (§4.6: Nota Débito + Factura nueva, Oficio 906781/2022 + Concepto 9156/2024) para el día que se integre facturación electrónica real — no se pierde el trabajo de investigación, solo se pospone su aplicación.
- **No se construye ninguna capa fiscal ahora** (regla de §4.4 sigue vigente sin cambios: `Pedido.total += diferencial` sigue siendo la única implementación).
- **Trigger explícito para retomar este punto**: el día que Agua Bambú decida conectar un proveedor de facturación electrónica real (Siigo/Alegra/Factus/otro) — en ese momento, y no antes, se valida con el contador y el proveedor el flujo exacto (pregunta 2 de §4.6) y se implementa la capa fiscal como adicional al mecanismo comercial interno.

**Este documento no vuelve a este punto salvo que ese trigger ocurra.** No es un pendiente activo — es una decisión cerrada con una condición de reapertura explícita y verificable (integración de FE real), no una fecha ni una suposición.
...
| Fase 3 — UX/UI de Pedidos (A vs B) | 🟡 **DECIDIDO (A, rediseño integral) — Plan Técnico entregado, pendiente de GATE** | Equipo/PO | `docs/pedidos/00-plan-frontend-rediseno-integral.md` — 7 decisiones D1-D7 + 9 fases de PR pendientes de aprobación antes de ejecutar PR-1. §3.6 (dirección de diseño SAP Fiori/Stripe/Linear) ya incorporado como principio vinculante del plan |
| Diferencial — mecanismo comercial | 🟢 **DECIDIDO E IMPLEMENTADO** (N2, #195-199) | ADR/Plan Maestro | Ninguna |
| Diferencial — representación fiscal | 🟢 **CERRADO por ahora** (confirmado por el negocio, 2026-09-06, §4.7) | — | Agua Bambú NO emite hoy FE real ante la DIAN — no bloquea nada del desarrollo actual. Doctrina (Oficio 906781/2022 + Concepto 9156/2024: Nota Débito + Factura nueva) queda documentada para cuando se integre un proveedor de FE real; ese es el único trigger de reapertura |
...
### 7.5 Revisión adversarial de la actualización 2026-09-06 (5ª ronda — cierre fiscal confirmado por el negocio)

- **¿Se cerró el punto fiscal con una respuesta inventada, en vez de la respuesta real del dueño del negocio?** No — la respuesta ("No, todo es manual/interno por ahora") vino directamente del dueño, vía pregunta explícita; no se infirió ni se asumió.
- **¿El cierre convierte la doctrina DIAN general en una conclusión fiscal específica para Agua Bambú, violando lo que el equipo pidió explícitamente no hacer?** No — §4.7 es explícito en que la doctrina queda *documentada para el futuro*, no aplicada ahora; no se concluye "por lo tanto Agua Bambú debe hacer X" — se concluye "por ahora no aplica, y el trigger de cuándo sí aplica es verificable (conectar un proveedor de FE real), no una fecha ni una suposición".
- **¿Se pierde el trabajo de investigación (§4.6) al cerrar el punto?** No — permanece intacto y citado; el cierre de §4.7 es aditivo, no reemplaza ni resume la investigación.
- **¿Queda alguna decisión de este documento sin resolver que no sea la ejecución de Fase 3?** No — la matriz de §6 no tiene ninguna fila roja tras esta ronda; el único trabajo hacia adelante es el Plan Técnico de rediseño (§3.7), que es ejecución, no una decisión pendiente.

...
PENDIENTE de negocio (no de blueprint): validación de disponibilidad de mercancía (preventiva vs detectiva) · qué cuenta como "comprometido" · venta preferencial a empleado. Y **EVIDENCIA HISTÓRICA A RECUPERAR** (EH-1..EH-5). La **mentalidad + casos + criterios de éxito** de cómo VENTA_LIBRE aparece en el Hub (aparece pero no nace ahí; dos puntos de captura — ruta y conciliación; 4 dimensiones que no se colapsan; VL-01..VL-15 de experiencia + VL-A01..VL-A16 de autorización) están cerrados en **`docs/pedidos/VENTA_LIBRE_EXPERIENCIA_HUB_v1.0.md`** (insumo: `docs/pedidos/VENTA_LIBRE_AUDITORIA_CONTRATO_CODIGO_v1.0.md`). Requisitos de diseño derivados: RD-1 (identificar comprador en ruta), RD-2 (antifraude observa las ventas libres), RD-3 (disponibilidad real en el punto de captura), RD-4 (borrar la entidad inerte), RD-5 (vincular venta libre de "más demanda" al pedido original).

**Instrucción de cierre (2026-09-08) incorporada** — `VENTA_LIBRE_EXPERIENCIA_HUB_v1.0.md` §0/§0bis/§3bis: definición canónica (venta no respaldada por Pedido, dentro del contexto de un Embarque); matriz de autorización (solo repartidor asignado en ruta — validado en backend — o Admin/Asistente **dentro de la conciliación de ese Embarque**; sin alta genérica); **el Pedido Hub NO crea VENTA_LIBRE** (`mostrar ≠ crear`; "Venta durante la ruta →" de Fase 5 solo navega — `hub-accion-frontera.test.ts`). Nueva **BRECHA-8** en la auditoría (ADMIN/ASISTENTE pueden crear VL contra cualquier Embarque `ABIERTO` sin gate de conciliación). Regla transversal: una señal de diferencia/fraude **no** es automáticamente una deuda del cliente → **`docs/pedidos/POLITICA_SALDO_FAVOR_Y_DIFERENCIAL_NEGATIVO_v1.0.md`** (política de dominio para el saldo a favor por diferencial negativo: compensación trazable, no borrado de historia; el "no se revierte automáticamente" de Fase 5 pasa a ser BRECHA PLAN↔CÓDIGO).
...
### 9.2 `GET /api/pedidos/[id]` — ✅ extendido (Fase 4b)
Antes incluía solo `factura` (lazy) + enriquecimiento de cliente/negocio. **Resuelto en Fase 4b** (rama `feat/pedidos-hub-4b`): el GET incluye ahora `pendienteN2` (`ObligacionPendiente` + `Actividad[]`), `embarqueResumen`, `pedidosVinculados` (G11.B, self-relation `pedidoOrigenId`) y `casosAbiertos`. Aditivo, solo lectura (guardrail estático lo verifica), sin cambio de schema. Contrato en `02-api-contract-pedidos.md`; tipo `PedidoPeekExtras`.
...
| C3 | El peek capa 2 necesita datos que `GET /api/pedidos/[id]` no devolvía (N2, relaciones) | Gap de contrato additivo — **RESUELTO (Fase 4b)** | §9.2 — endpoint extendido, sin schema, solo lectura |
